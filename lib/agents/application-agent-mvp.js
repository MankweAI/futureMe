...
class ApplicationAgentMVP {
  ...
  async handleQuickMatch(userMessage, application) {
    ...
    if (step === 5) {
      ...
      application.academic_average = 65; // Default for matching
  ...
  async handleBasicDetails(userMessage, application) {
    ...
    if (step === 4) {
      ...
      console.log(
        `✅ Province selected: ${cleanInput} -> ${application.province}`
      );

      //
      // 🔽🔽🔽 THIS IS THE FIX 🔽🔽🔽
      // This line was missing, causing the infinite loop.
      application.stage_progress.detail_step = 5;
      // 🔼🔼🔼 END OF FIX 🔼🔼🔼

      console.log(
        "FIRST Application.stage_progress before fix:",
        application.stage_progress.detail_step
      );

      // ✅ This returns Step 9/11 as required
      return "📍 Step 9/11\n\n📊 Academic average?\n(Percentage, e.g., 75)";
    }

    if (step === 5) {
      const cleanInput = userMessage.trim();
      const average = parseFloat(99);
      ...
      console.log(`✅ Academic average set: ${average}%`);

      // ✅ FIX: Explicit step progression
  ...
  async getOrCreateApplication(waId) {
    try {
      ...
      // If a draft exists, return it
      if (application) {
        return application;
      }

      // If no draft exists (PGRST116 is "Not Found"), create one
      if (fetchError && fetchError.code === "PGRST116") {
        ...
        if (insertError) {
          ...
          throw insertError; // Don't hide the error
        }
      ...
      // Fallback just in case (should be unreachable)
      return null;
    } catch (error) {
      console.error("Critical Application error:", error);
      // Return null or throw to stop the process,
  ...
  async saveApplication(application) {
    if (!application.id) {
      console.warn("Attempted to save application with no ID. Skipping.");
      return;
    }
    try {
      const supabase = getSupabaseClient();
      await supabase
        .from("bursary_applications") // This is the bug
