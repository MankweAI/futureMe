...
class OnboardingAgent {
  ...
  async processMessage(userMessage, session, userStatus) {
    ...
    if (currentStage === STAGES.START) {
      // --- FIX 3: WELCOME MESSAGE REFORMATTED ---
      response =
    ...
    // --- Subsequent Steps ---
      ...
      switch (lastStage) {
        ...
        case STAGES.AWAIT_INTENT:
          ...
          nextStage = STAGES.AWAIT_DENOMINATION;
          break;
        ...
        case STAGES.AWAIT_DENOMINATION:
          ...
          // --- FIX 1: DENOMINATION QUESTION (OPTIONS) ---
          response =
            "Great! We all worship in different ways. Which of these best describes your faith background?\n\n1️⃣ Mainstream (e.g., Baptist, Methodist)\n2️⃣ Charismatic / Pentecostal\n3️⃣ Non-denominational\n4️⃣ Just a follower of Christ\n5️⃣ Other / Still exploring";
          nextStage = STAGES.AWAIT_RHYTHM;
          break;

        case STAGES.AWAIT_RHYTHM:
          profileData.denomination = userMessage.trim(); // Saves the number (e.g., "1")
          response =
          ...
          if (profileData.intent === "Prayer Partner") {
            nextStage = STAGES.AWAIT_PRAYER_STYLE;
          } else if (profileData.intent === "Fellowship & Friends") {
            nextStage = STAGES.AWAIT_FELLOWSHIP_INTEREST;
          }
          break;
        ...
        case STAGES.AWAIT_PRAYER_STYLE:
          ...
          nextStage = STAGES.AWAIT_MATCH_GENDER_PREF;
          break;
        ...
        case STAGES.AWAIT_MATCH_AGE_PREF:
          ...
          // --- FIX 2: AGE RANGE QUESTION (CLEARER) ---
          response = `And what age range for connections? (You are ${profileData.age})\n\n1️⃣ 18-25\n2️⃣ 26-35\n3️⃣ 36-45\n4️⃣ 46+\n5️⃣ Open to all ages`;
          nextStage = STAGES.COMPLETE; // Now we are ready
          break;

        // --- Phase 5: COMPLETE / Celebration ---
        case STAGES.COMPLETE:
          profileData.match_age_pref = userMessage.trim(); // Saves the number (e.g., "1")
